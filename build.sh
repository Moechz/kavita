...
# 产物命名（2026-09 用户多次真机实测纠正）：一律用带版本号的 Debian 架构名
#   kavita_<完整版本>_<amd64|arm64>.deb + .sha256
# 此前"App Center 手动安装页拒绝 *amd64* 文件名"的说法经验证不成立，
# 不再额外产出 kavita_x86_64.deb 之类的第二份拷贝（单包更清晰）
...
PYEOF
  ...
  # 注意：TOS 系统 dpkg 配置 path-exclude=/usr/share/doc/*（仅豁免
  # copyright 与 changelog.*），放 doc 目录会被平台丢弃；故放应用目录
  log "  + PROVENANCE.md（源码构建审计链 → /usr/local/$APP_ID/）"
  cp "$ASSETS_DIR/PROVENANCE.md" "$APP/PROVENANCE.md"
...
PYEOF
  ...
  log "校验 .lang（23 语超集齐全 + 官文长度上限 + beta 门禁）..."
  local lang_missing
...
required = ["zh-cn","zh-hk","en-us","fr-fr","de-de","it-it","es-es",
            ...
            "th-th","vi-vn"]
errs = [t for t in required if f"[{t}]" not in text]
# 官方 app.lang 硬上限（字符）：name64 / auth64 / descript512 / important512
LIM = {"name": 64, "auth": 64, "descript": 512, "important": 512}
secs = re.split(r'^\[([a-z]{2}-[a-z]{2})\]$', text, flags=re.M)
for i in range(1, len(secs), 2):
    code, body = secs[i], secs[i+1]
    for fld, lim in LIM.items():
        m = re.search(rf'^{fld}\s*=\s*"(.*)"$', body, re.M)
        v = m.group(1) if m else ""
        if not v:
            errs.append(f"{code}:{fld} 为空")
        elif len(v) > lim:
            errs.append(f"{code}:{fld}={len(v)}>{lim}")
    # 版本三处一致性（config.ini / control / app.lang）
    m = re.search(r'^version\s*=\s*"(.*)"$', body, re.M)
    if m and m.group(1) != want_ver:
        errs.append(f"{code}:version={m.group(1)}!={want_ver}")
    if re.search(r'\bbeta\b', body, re.I):
        errs.append(f"{code}:含 beta")
print(",".join(errs))
PYEOF
)
  [ -z "$lang_missing" ] || { warn "lang 校验失败: $lang_missing"; fail=1; }
...
PYEOF
  ...
  n_net=$(grep -rniE 'pip(3)? install|pip download|--index-url|pypi\.(org|tuna)|urllib\.request|urlopen|npm install -g|curl .*(debian|ubuntu)\.org' \
      ...
      --include='postinst' --include='prerm' --include='postrm' 2>/dev/null | wc -l | tr -d ' ' || true)
...
stage_deb() {
  ...
  # 随包 sha256（Release/商店提交都要求校验文件；清单内只写文件名）
  local base
  ...
  log "完成: $DEB_FILE（+ .sha256）"
  log "Release tag 须为 v$VERSION_FULL"
}
...
产物           : $DEB_FILE + .sha256（Release tag: v$VERSION_FULL）
EOF
}
